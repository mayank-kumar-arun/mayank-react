...
const RdsCompClientBasic = (props: RdsCompClientBasicProps) => {
	...
	const clientUrlhandleChange = (event: {
		target: { value: React.SetStateAction<string> };
	}) => {
	...
	const logoUrlhandleChange = (event: {
		target: { value: React.SetStateAction<string> };
	}) => {
	...
	const isFormValid =
		isFullnameValid(fullname) && isEmailValid(email) && isMessageValid(message);
		...
		<>
			<div>
				<form onSubmit={handleSubmit}>
					...
					<RdsCheckbox label="Required Consent" checked={false} />
				</form>
			</div>
		</>
	);
