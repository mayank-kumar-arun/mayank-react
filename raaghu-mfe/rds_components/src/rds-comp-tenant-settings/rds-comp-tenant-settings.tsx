...
const RdsCompTenantSettings = (props: RdsCompTenantSettingsProps) => {
	...
	const isPasswordInputEmptyAndTouched = isPasswordTouched && isEnteredPasswordEmpty;
	const isPasswordInputInvalid = isEnteredPasswordInvalid && isEnteredPasswordEmpty;
	
	const [enteredConfirmPassword, setEnteredConfirmPassword] = useState("");
	const [isConfirmPasswordTouched, setIsConfirmPasswordTouched] = useState(false);
	const [isConfirmPasswordFocused, setIsConfirmPasswordFocused] = useState(false);
	const isEnteredConfirmPasswordEmpty = enteredConfirmPassword === "";
	const isEnteredConfirmPasswordInvalid = !passwordRegex.test(enteredConfirmPassword);
	const isConfirmPasswordInputEmptyAndTouched = isConfirmPasswordTouched && isEnteredConfirmPasswordEmpty;
	const isConfirmPasswordInputInvalid = isEnteredConfirmPasswordInvalid && isEnteredConfirmPasswordEmpty;
	const isPasswordMatch = isConfirmPasswordFocused && !isEnteredConfirmPasswordEmpty? (enteredPassword === enteredConfirmPassword):true ;
		...
		<div>
			<div className="tab-content py-4">
				<form>
					...
					{!hostDatabaseChecked && <div className="row mb-3">
					...
					{!isRandomPasswordChecked && <div className="row mb-3">
						<div className="col-md-6 sm-p-0">
							<div className="form-group mb-3">
								<RdsInput
									inputType="password"
                ...
                {isEnteredPasswordInvalid && !isEnteredPasswordEmpty && (
                  <span className="red-color-error">
                    Password should be between 8 to 15 characters which contain atleast one lowercase letter, one uppercase letter, one numeric digit, and one special character.
                  </span>
                )}
							</div>
						</div>
				...
				{!isPasswordMatch && <span className="red-color-error">
							...
							<div className="col-md-12 sm-p-0">
								<div className="form-group mb-3">
									<RdsCheckbox
										...
										onChange={(e)=> setIsRandomPasswordChecked(e.target.checked)}									
										checked={false}
									></RdsCheckbox>
								</div>
							</div>
