...
import { RdsIcon, RdsCheckbox, RdsButton, RdsPagination } from '../rds-elements'
export interface RdsCompDatatableProps {
    ...
    onSortSelection(arg: { sortClickEvent: MouseEvent<HTMLSpanElement, globalThis.MouseEvent>; sortOrder: string; }): any;
}
const RdsCompDatatable = (props: RdsCompDatatableProps) => {
    ...
    const onSortClickHandler = (event: MouseEvent<HTMLSpanElement, globalThis.MouseEvent>, sortOrder: string) => {
                    ...
                    <>
                                ...
                                <>
                                    ...
                                    {props.tableHeaders.map((tableHeader, index) =>
                                        <th key={"tableHeader-" + index}>
                                            ...
                                            {tableHeader.sortable && <span className='px-2'>
                                                <span className='btn btn-sm px-0' onClick={(e)=>onSortClickHandler(e,'ascending')}><RdsIcon name={'up'} height='12px' width='12px' stroke={true} /></span>
                                                <span className='btn btn-sm px-0'><RdsIcon name={'down'} height='12px' width='12px' stroke={true} /></span>
                                            </span>}
                                        </th>
                                    )}
                        ...
                        <tbody>
                            {props.tableData.map((tableDataRow, index) =>
                                (props.pagination && index >= rowStatus.startingRow && (typeof rowStatus.endingRow != "undefined") && index < rowStatus.endingRow) &&
                                                ...
                                                <div className="dropdown">
                                                    ...
                                                    <ul className="dropdown-menu RdsCompDataTable__Actions__Ul">
                                                        {props.actions.map((action, actionIndex) =>
                                                            <li key={"action-" + actionIndex + "-inside-tableRow" + index}>
                                                                <a
                                                                    onClick={(e) => { actionOnClickHandler(e, tableDataRow, index, action) }}
