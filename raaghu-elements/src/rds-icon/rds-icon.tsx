...
const RdsIcon = (props: RdsIconProps) => {
  ...
  const svgElementFromString =(svgContent: string): SVGElement => {
    ...
    return svg || document.createElementNS('http://www.w3.org/2000/svg', 'path');
   
  }
  var stringData = (icon!=undefined)?svgElementFromString(icon).outerHTML:'' ;

  return <span className={props.classes} dangerouslySetInnerHTML={{__html:stringData}} />;;
};
